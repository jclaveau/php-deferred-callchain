...
class DeferredCallChain implements \JsonSerializable, \ArrayAccess
{
    ...
    public final function __call($method, array $arguments)
    {
        ...
        $this->stack[] = ['method' => $method, 'arguments' => $arguments, 'file' => isset($caller['file']) ? $caller['file'] : null, 'line' => isset($caller['line']) ? $caller['line'] : null];
        return $this;
    }
    /**
    ...
    {
        $caller = debug_backtrace(DEBUG_BACKTRACE_IGNORE_ARGS, 1)[0];
        $this->stack[] = ['entry' => $key, 'file' => isset($caller['file']) ? $caller['file'] : null, 'line' => isset($caller['line']) ? $caller['line'] : null];
        return $this;
    }
    /**
     ...
     */
    protected function checkMethodIsReallyCallable($method_type, &$current_chained_subject, $method_name, $arguments)
    {
        $is_called = true;
        try {
            if ($method_type == '->') {
     ...
     */
    protected function exceptionTrownFromMagicCall($trace, $current_chained_subject, $method_name)
    {
        // Before PHP 7, there is a raw for the non existing method called
        $call_user_func_array_position = PHP_VERSION_ID < 70000 ? 2 : 1;
        return ($trace[0]['function'] == '__call' || $trace[0]['function'] == '__callStatic') && $trace[0]['class'] == (is_string($current_chained_subject) ? $current_chained_subject : get_class($current_chained_subject)) && $trace[0]['args'][0] == $method_name && ($trace[$call_user_func_array_position]['file'] == __FILE__ && $trace[$call_user_func_array_position]['function'] == 'call_user_func_array');
    }
    /**
